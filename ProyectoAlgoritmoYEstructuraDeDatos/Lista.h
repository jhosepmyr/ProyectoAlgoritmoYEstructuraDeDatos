...
T Lista<T>::obtenerEliminarPorPosicion(uint pos) {
    ...
    if (pos == 0) {
        // Si la posici�n es la primera, eliminamos el primer elemento y lo retornamos
        T elemento = ini->elem;
        Nodo* temp = ini;
        ini = ini->sig;
        delete temp;
    ...
    // Guardamos el elemento en la posici�n especificada
    T elemento = aux->sig->elem;

    // Eliminamos el elemento de la posici�n
    Nodo* temp = aux->sig;
    aux->sig = temp->sig;
    delete temp;
    lon--;
...
T Lista<T>::eliminarPrimero() {
    if (lon == 0) {
        throw std::runtime_error("La lista est� vac�a, no se puede eliminar el primer elemento.");
    }
...
T Lista<T>::eliminarUltimo() {
    if (lon == 0) {
        throw std::runtime_error("La lista est� vac�a, no se puede eliminar el �ltimo elemento.");
    }
...
void Lista<T>::eliminarTodos() {
    ...
    ini = nullptr;  // Establecemos el puntero inicial a nullptr para indicar que la lista est� vac�a.
...
int Lista<T>::Particion(int p, int r) {
    ...
    int i = p - 1; // �ndice de los menores
    for (int j = p; j < r; j++) {
        if (obtenerPos(j).getPrecio() <= x.getPrecio()) {
            i++;
            intercambiar(i, j); // Utiliza una funci�n para intercambiar elementos
        }
    }
    intercambiar(i + 1, r); // Utiliza una funci�n para intercambiar elementos
    return i + 1;
}
...
T Lista<T>::QuickSelect(int p, int r, int k) {
    ...
    // �ndice del pivote con A ordenado Izq(Menores) Der(Mayores) al pivote
    int q = Particion(p, r);
    int l = q - p + 1; // N�mero de elementos del subarreglo donde se encuentra el k-�simo elemento
    if (k == l) {
        return obtenerPos(q);
    }
    else if (k < l) {
